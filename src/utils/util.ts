...
const createOnSearch = async (req: Request) => {
    ...
    if ((!config.ENABLE_LOCATION_SERVICES) && (start_codes.length === 0 || end_codes.length === 0)) {
        var all_stops = await getAllStations();
        for (var stop of all_stops) {
            var location = await createLocationObject(stop.stop_id);
        ...
        for (var end_code of end_codes) {
            ...
            if (stop_times.length !== 0) {
                ...
                if (!_.find(locations, ['id', start_code])) {
                    const this_locations = await createLocationObject(start_code);
                    locations.push(this_locations);
                }
                if (!_.find(locations, ['id', end_code])) {
                    const this_locations = await createLocationObject(end_code);
                    locations.push(this_locations);
                }
...
const createLocationObject = async (code: string) => {
